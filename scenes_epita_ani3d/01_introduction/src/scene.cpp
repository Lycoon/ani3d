...
void scene_structure::initialize()
{
	...
	// Cones
	trunk.initialize_data_on_gpu(mesh_primitive_cylinder(0.05f, {0,0,0}, {0,0,0.25f}));
	trunk.material.color = { 0.5f, 0.5f, 0 };

	cone.initialize_data_on_gpu(mesh_primitive_cone(0.1f, 0.2f));
	cone.material.color = { 0, 1.0f, 0 };

	const int N_cone = 40;
	for (int k = 0; k < N_cone; ++k)
	{
		float x = rand_interval(-2, 2);
		float z = rand_interval(-2, 2);

		cgp::vec3 pos = { x, z, 0 };
		cone_positions.push_back(pos);
	}
...
void scene_structure::display_frame()
{
	...
	for (int k = 0; k < cone_positions.size(); ++k)
	{
		...
		trunk.model.translation = cone_positions[k];
		trunk.model.translation.z -= 0.2f;
